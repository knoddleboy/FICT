...
static void server(int listenfd) {
        int connfd;
        pid_t pid;

        pid = getpid();
        printf("Server %jd started\n", (intmax_t)pid);
        for (;;) {
                connfd = accept(listenfd, NULL, 0);
                if (connfd < 0)
                        exit_err("accept()");
                printf("Server %jd accepted connection from\n", (intmax_t)pid);
...
int main(void) {
        ...
        unsigned int serv_num = 3;
        unsigned int serv_forked;
        int listenfd, opt;
        pid_t pid;
        ...
        if (serv_forked == 0) {
                warn_errx("no server was created");
                return EXIT_FAILURE;
        }

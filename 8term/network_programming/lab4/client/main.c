...
int main(int argc, char *argv[]) {
        if (argc != 5) {
                errx(EXIT_FAILURE, "Usage: %s <server_address> <server_port> <file_name> <max_size>", argv[0]);
        }
        ...
        if (strlen(file_name) >= MAX_FILENAME_LEN)
                errx(EXIT_FAILURE, "File name is too long (max %d characters)", MAX_FILENAME_LEN - 1);
